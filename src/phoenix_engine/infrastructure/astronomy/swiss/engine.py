...
class SwissEngine:
    ...
    def rise_trans(
        ...
        rsmi: int,
        geopos: tuple,
        atpress: Optional[float],
        attemp: Optional[float],
    ) -> Tuple[float, int]:
        """
        Portable wrapper:
        ...
        strategies = []
        if atpress is None and attemp is None:
            strategies.append(
                ("pyjhora_kw", lambda: swe.rise_trans(jd_ut, body_id, geopos=geopos, rsmi=rsmi))
            )

        atpress_val = 0.0 if atpress is None else atpress
        attemp_val = 0.0 if attemp is None else attemp

        # Strategy A: "mutant" 6-arg signature (no starname, no ephe_flags)
        strategies.extend([
            ("mutant_6", lambda: swe.rise_trans(jd_ut, body_id, rsmi, geopos, atpress_val, attemp_val)),
            # Strategy B: modern 8-arg signature
            ("modern_8", lambda: swe.rise_trans(jd_ut, body_id, "", ephe_flags, rsmi, geopos, atpress_val, attemp_val)),
            # Strategy C1: legacy 7-arg signature with starname as string
            ("legacy_7_str", lambda: swe.rise_trans(jd_ut, body_id, "", rsmi, geopos, atpress_val, attemp_val)),
            # Strategy C2: legacy 7-arg signature with starname as int (some builds expect int)
            ("legacy_7_int", lambda: swe.rise_trans(jd_ut, body_id, 0, rsmi, geopos, atpress_val, attemp_val)),
        ])
